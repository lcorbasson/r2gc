class Admin::ToolPrivatePhotosController < Admin::ResourceController
  ...
  def create
    ...
    @asset = @tool.tool_private_photos.new(params[:tool_private_photo])
    if @asset.save
  ...
  def edit
    tool_id = params[:software_id] ? params[:software_id] : params[:test_engine_id] ? params[:test_engine_id] : params[:tool_engine_id] ? params[:tool_engine_id] : params[:measuring_engine_id]
    @tool = Tool.find(tool_id)
    @asset = @tool.find(params[:tool_private_photo_id])
  end

  def update
    @tool = Tool.find(params[:tool_id])
    @asset = @tool.find(params[:tool_private_photo_id])
    if @asset.update_attributes(params[:tool_private_photo])
