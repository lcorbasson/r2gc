class ToolEnginesController < SiteController
  ...
  def index
    ...
    respond_to do |format|
      ...
      format.csv  { send_data(tool_engines_to_csv.read, :type => 'text/csv; header=present', :filename => 'machines-outils.csv') }
    end
  end
  ...
  def to_pdf
    @tool_engine = ToolEngine.find(params[:id])
    prawnto :filename =>  "#{@tool_engine.name.remove_accent}.pdf", :inline => true, :template => "to_pdf.pdf.prawn"
  end
