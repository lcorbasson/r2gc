class TestEnginesController < SiteController
  ...
  def index
    @search = TestEngine.without_deleted.search(params[:search])
    ...
    respond_to do |format|
      ...
      format.csv  { send_data(test_engines_to_csv.read, :type => 'text/csv; header=present', :filename => 'moyens_dessais.csv') }
    end
  end
